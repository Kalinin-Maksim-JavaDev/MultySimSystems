...
public class CollisionSpace implements CollisionContext {
    ...
    public void add(Body body) {
        ...
        //Kalinin MP
        ((AbstractShape) body.getShape()).bound = new int[(int)((AbstractShape) body.getShape()).getBounds().getHeight()+2][2];
        ((AbstractShape) body.getShape()).l = new int[((AbstractShape) body.getShape()).bound.length];
        ((AbstractShape) body.getShape()).bounseted = new boolean[((AbstractShape) body.getShape()).bound.length];
        ((AbstractShape) body.getShape()).FillMap(map, body);
        //Kalinin MP
    }
        ...
        for (int i = 0; i < bodyList.size(); ++i) {
            ...
            for (int j = i + 1; j < bodyList.size(); ++j) {
                ...
                if (newArb.getNumContacts() > 0) {
                    ...
                    //System.out.println(totalTime+": "+bi.toString()+""+bj.toString());
                } else {
     ...
     */
    //Kalinin MP
    public void resolveWithHash(BodyList bodyList, float dt) {

        BodyList bodyList_ = new BodyList();
        AABox bound_;
        ArrayListPure maskY;
        ArrayListPure maskX;
        int y_ = 0;
        ...
        for (int i = 0; i < bodyList.size(); ++i) {
            ...
            {
                maskY = ((AbstractShape) bi.getShape()).maskY;
                maskX = ((AbstractShape) bi.getShape()).maskX;

                for (int i_ = 0; i_ < maskY.size(); i_++) {
                    y_ = ((Integer) maskY.get(i_)).intValue();
                    x_ = ((Integer) maskX.get(i_)).intValue();
                    if ((x_ > -1) && (y_ > -1) && (x_ < map[0].length) && (y_ < map.length)) {
                        bj = map[y_][x_];
                        if ((bj != null) && (bj != bi)) {
                            if (!bodyList_.contains(bj)) {
                                bodyList_.add(bj);
//                                if (bodyList_.size()>2){
//                                    break;
//                                }
            ...
            for (int j = 0; j < bodyList_.size(); j++) {
                bj = bodyList_.get(j);
                if (bj.disabled()) {
                    continue;
                }

                if ((bj.z!=0)&&(bi.z!=0)&&(bj.z!=bi.z)) {
                    continue;
                }
                if (bitmaskExcludes) {
                    if ((bi.getBitmask() & bj.getBitmask()) != 0) {
